...
# Вспомогательная функция для экранирования пользовательского ввода
def safe_html(text):
    if not text:
        return ""
    return html.escape(str(text))

# --- РАБОТА С БД (СОВМЕСТИМО С GUNICORN) ---
def init_db():
    with sqlite3.connect('ggkassa_main.db', timeout=10) as conn:
        ...
        # Устанавливаем процент по умолчанию = 3.0%, если он еще не задан
...
def get_ref_percent():
    """Получает текущий процент реферальной системы из БД"""
    with sqlite3.connect('ggkassa_main.db', timeout=10) as conn:
        c = conn.cursor()
...
def set_ref_percent(percent_val):
    """Обновляет процент реферальной системы в БД"""
    with sqlite3.connect('ggkassa_main.db', timeout=10) as conn:
        c = conn.cursor()
            ...
            if dep:
                ...
                if ref and ref[0]:
                    ...
                    try:
                        bot.send_message(
                            ref[0], 
                            f"<b>💰 Ваш друг пополнил счет! Вам начислено {bonus:.2f} сом реферального бонуса ({ref_pct:g}%).</b>", 
                            parse_mode='HTML'
                        )
                    except Exception:
                        pass
...
def admin_menu():
    ...
    markup.add(f"🔗 Процент реф ({ref_pct:g}%)", "📢 Рассылка")
...
def platform_markup(action_prefix):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("⚽️ 1xBet", callback_data=f"{action_prefix}_1xbet"),
        types.InlineKeyboardButton("🎰 Melbet", callback_data=f"{action_prefix}_melbet")
    )
    return markup
...
🌀 Пополнения и Выводы: <b>1xBet</b> и <b>Melbet</b>
🟠 Без процентов
...
def referrals_menu(msg):
    ...
    text = f"""🔥 <b>Реферальная Система GGKASSA</b>
...
def deposit(msg):
    ...
    bot.send_message(
        msg.chat.id, 
        "<b>Выберите платформу для пополнения:</b>", 
        parse_mode='HTML', 
        reply_markup=platform_markup("dep_plat")
    )
...
def get_account_id(msg):
    ...
    platform_name = temp_data.get(msg.chat.id, {}).get("platform", "1xBet / Melbet")
    account_val = f"{platform_name} | {msg.text.strip()}"
    
    temp_data[msg.chat.id]["account_id"] = account_val
    bot.send_message(msg.chat.id, f"<b>💰 Введите сумму для пополнения {platform_name} (от 100 до 100 000 сом):</b>", parse_mode='HTML', reply_markup=back_menu())
...
def withdraw_start(msg):
    ...
    bot.send_message(
        msg.chat.id, 
        "<b>Выберите платформу для вывода средств:</b>", 
        parse_mode='HTML', 
        reply_markup=platform_markup("w_plat")
    )
...
def withdraw_get_elqr(msg):
    ...
    bot.send_message(msg.chat.id, f"<b>Шаг 2:</b> 🆔 Введите ID счета {platform_name}:", parse_mode='HTML', reply_markup=back_menu())
...
def withdraw_get_id_text(msg):
    ...
    platform_name = temp_data.get(msg.chat.id, {}).get("platform", "1xBet / Melbet")
    temp_data[msg.chat.id]["id_photo"] = f"{platform_name} | {msg.text.strip()}"
    
    bot.send_message(msg.chat.id, f"✉️ <b>Шаг 3:</b> После оформления заявки на {platform_name} пришлите полученный <b>код подтверждения</b> боту:", parse_mode='HTML', reply_markup=back_menu())
...
def ref_withdraw_get_amount(msg):
    ...
    bot.send_message(msg.chat.id, "🆔 Введите ваш <b>ID счета 1xBet / Melbet</b> для зачисления реферальных средств:", parse_mode="HTML", reply_markup=back_menu())
...
def ref_withdraw_get_id(msg):
    ...
    for admin in admins:
        try:
            bot.send_message(admin, f"👥 <b>ЗАЯВКА НА ВЫВОД РЕФЕРАЛЬНЫХ #{rw_id}</b>\n\n👤 От: {user_id}\n💰 Сумма: {amount:,.2f} сом\n🎯 На ID: <code>{safe_html(target_id)}</code>", parse_mode="HTML", reply_markup=markup)
        except Exception:
            pass
...
# Управление процентом реферальной системы через админку
@bot.message_handler(func=lambda m: m.text.startswith("🔗 Процент реф") and (m.from_user.id in get_admins() or m.from_user.id == MAIN_ADMIN))
def change_ref_percent_start(msg):
    current_pct = get_ref_percent()
    bot.send_message(
        msg.chat.id, 
        f"🔗 <b>Изменение реферального процента</b>\n\nТекущее значение: <b>{current_pct:g}%</b>\n\nВведите новый процент (число, например: <code>3</code> или <code>5</code>):", 
        parse_mode='HTML', 
...
def process_save_ref_percent(msg):
    ...
    try:
        ...
        bot.send_message(msg.chat.id, f"✅ Новый реферальный процент <b>{new_pct:g}%</b> успешно установлен!", parse_mode='HTML', reply_markup=admin_menu())
    except Exception:
        bot.send_message(msg.chat.id, "❌ Ошибка! Введите число от 0 до 100.", reply_markup=back_menu())
        bot.register_next_step_handler(msg, process_save_ref_percent)
...
def handle_call(call):
    if call.data.startswith("dep_plat_"):
        platform = "1xBet" if call.data == "dep_plat_1xbet" else "Melbet"
        ...
        bot.send_message(
            ...
            f"<b>🆔 Введите ID счета {platform}:</b>", 
            parse_mode='HTML', 
            reply_markup=back_menu()
        )
    ...
    if call.data.startswith("w_plat_"):
        platform = "1xBet" if call.data == "w_plat_1xbet" else "Melbet"
        temp_data[call.message.chat.id] = {"platform": platform}
        ...
        instruction = f"""📌 <b>Как вывести средства с {platform}</b>
...
💳 <b>Шаг 1:</b> Прикрепите ваш <b>ELQR</b> (фотографией):"""
        ...
        bot.send_message(call.message.chat.id, instruction, parse_mode='HTML', reply_markup=back_menu())
        bot.register_next_step_handler(call.message, withdraw_get_elqr)
        return
...
⏱️ <b>Закрыта за:</b> {elapsed_time}s"""
            ...
            try:
                bot.edit_message_caption(
                    caption=f"✅ ЗАЯВКА НА ПОПОЛНЕНИЕ #{dep_id} ОДОБРЕНА", 
        ...
        if result:
            ...
            try:
                bot.edit_message_caption(
                    caption=f"❌ ЗАЯВКА НА ПОПОЛНЕНИЕ #{dep_id} ОТКЛОНЕНА", 
                    chat_id=call.message.chat.id, 
                    message_id=call.message.message_id
                )
            except Exception:
...
# --- СТАБИЛЬНЫЙ ЗАПУСК БОТА С LOCK-ПОРТАМ ---
_lock_socket = None
...
def run_bot():
    print("🚀 Инициализация и запуск Telegram-бота GGKASSA 24/7...")
    try:
        bot.remove_webhook()
    except Exception as e:
