...
# Вспомогательная функция для экранирования пользовательского ввода (защита от разрыва HTML-тегов)
def safe_html(text):
    if not text:
        return ""
...
# --- ВЕБ-ИНТЕРФЕЙС FLASK (ДЛЯ ХОСТИНГА И UPTIMEROBOT 24/7) ---
@app.route('/')
def home():
...
@app.route('/health')
def health_check():
    """Эндпоинт проверки здоровья для Render и UptimeRobot"""
    try:
        s = get_stats()
        active = is_bot_active()
        return {
