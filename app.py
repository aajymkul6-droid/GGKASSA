...
MAIN_ADMIN = 7905866942  # Новый ID Главного Администратора

SUPPORT = "@KONZ_T"
BOT_USERNAME = "BMkassa_bot"
BOT_NAME = "BM KASSA"
...
# --- РАБОТА С БД ---
def init_db():
    with sqlite3.connect('bmkassa_main.db', timeout=10) as conn:
        c = conn.cursor()
            ...
            if dep:
                ...
                if ref and ref[0]:
                    ...
                    try:
                        bot.send_message(
                            ref[0], 
                            f"<b>{EMOJI['money']} Ваш друг пополнил счет! Вам начислено {bonus:.2f} сом реферального бонуса ({ref_pct:g}%).</b>", 
                            parse_mode='HTML'
                        )
                    except Exception:
                        pass
...
def get_account_id(msg):
    ...
    bot.send_message(msg.chat.id, f"<b>{EMOJI['money']} Введите сумму для пополнения 1xBet (от 100 до 100 000 сом):</b>", parse_mode='HTML', reply_markup=back_menu())
